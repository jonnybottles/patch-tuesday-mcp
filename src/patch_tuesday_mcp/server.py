...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "Microsoft-stated supersedence links (which KBs it replaces), "
        "include_kb_details=True for per-KB fixed builds and restart "
        "requirements, include_known_issues=True for the issues Microsoft "
        "has publicly confirmed the update introduces (symptoms, workarounds, "
        "resolving KB), best-effort from the KB's support page — published "
        "mainly for Windows updates; other products usually report "
        "none_published — and include_update_summary=True for what an update "
        "changes (the KB page's summary and improvements highlights, same "
        "page, one fetch). Set include_stats=True with limit=0 for a month "
